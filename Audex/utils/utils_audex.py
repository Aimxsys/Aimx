...
class Aimx:
    class Paths:
        ...
        GEN_PLOTS_GENUM  = os.path.join(WORKDIR, "gen_plots_genums")
        GEN_PLOTS_VENCS  = os.path.join(WORKDIR, "gen_plots_vencs")
...
def load_traindata(arg_traindata_path):
    ...
    inputs = np.array(traindata[Aimx.TrainData.SIGNUMS]) # x: json list to numpy array (MFCCs  turn into a 3d array)
...
def plot_interpret_as_melspec(data, plot_title):
    print_info("Plotting melspec of shape {}...".format(pinkred(data.shape)))
    log_mel_spectrogram = librosa.power_to_db(data)
...
def plot_signal(signal, signal_filename, windowtitle, ylim=None):
    if ylim is None:
        ylim = 1 if np.max(signal) < 1 else np.max(signal)
    pt.figure(figsize=(20, 12)).canvas.set_window_title(windowtitle)
...
def plot_ae_signals_single_chart(signals, original_filename, modelname, ylim=None):
